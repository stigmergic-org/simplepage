...
import "./FoamIdenticon.sol";

contract TokenRendererV3 is ITokenRenderer {
    ISimplePage public pages;
    FoamIdenticon public foam;

    constructor(ISimplePage _pages) {
        pages = _pages;
        foam = new FoamIdenticon();
    }
    ...
    function renderPage(uint256 tokenId) public view returns (string memory) {
        ...
        uint256 activeUnits = _countActiveUnits(pageData.units);

        // Calculate dynamic height based on number of units
        uint256 unitsSectionHeight = pageData.units.length * 40 + 65; // 65 for header, spacing, and summary
        uint256 totalHeight = 450 - 170 + unitsSectionHeight; // 170 is the original units section height
        uint256 cardHeight = totalHeight - 50;
        ...
        uint256 summaryY = 280 + pageData.units.length * 40;
        svg = string(
            abi.encodePacked(
                svg,
                '<text x="65" y="',
                Strings.toString(summaryY),
    ...
    function _countActiveUnits(uint256[] memory units) internal view returns (uint256) {
        uint256 activeUnits = 0;
        for (uint256 i = 0; i < units.length; i++) {
            if (units[i] > block.timestamp) {
                activeUnits++;
            }
        }
        return activeUnits;
    ...
    function _buildSvgStart(
        string memory domain,
        uint256 totalHeight,
        uint256 unitsSectionHeight,
        uint256 cardHeight,
        string memory foamBase64
    ) internal pure returns (string memory) {
        string memory totalHeightStr = Strings.toString(totalHeight);
        string memory cardHeightStr = Strings.toString(cardHeight);
        string memory unitsSectionHeightStr = Strings.toString(unitsSectionHeight);
        ...
        svg = string(
            abi.encodePacked(
                svg,
                "<!-- Foam identicon background -->",
                '<image href="data:image/svg+xml;base64,',
                foamBase64,
                '" x="25" y="25" width="350" height="',
                cardHeightStr,
                '" preserveAspectRatio="xMidYMid slice" clip-path="url(#cardClip)" opacity="0.7"/>',
                "<!-- Main card container with foam background -->",
                '<rect x="25" y="25" width="350" height="',
                cardHeightStr,
                '" rx="24" ry="24" fill="white" fill-opacity="0.55" stroke="url(#borderGradient)" stroke-width="1.5" filter="url(#premiumShadow)"/>',
                "<!-- Premium header -->",
    ...
    function _appendUnitRows(string memory svg, uint256[] memory units) internal view returns (string memory) {
        uint256 yPos = 265;
        for (uint256 i = 0; i < units.length; i++) {
            string memory status = units[i] > block.timestamp ? "Active" : "Expired";
            string memory statusColor = units[i] > block.timestamp ? "#059669" : "#dc2626";

            // Convert timestamp to ISO format
            string memory isoDate = _timestampToISO(units[i]);
